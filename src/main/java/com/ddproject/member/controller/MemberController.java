...
public class MemberController {
	...
	public ResponseEntity<ApiResponse<String>> updateMeber(@PathVariable Long memberId,
		...
		try {
			...
			return ResponseEntity.ok(ApiResponse.success("닉네임이 성공적으로 수정되었습니다. 새 닉네임: " + memberResponseDto.getNickname()));
		} catch (AccessDeniedException e) {
			return ResponseEntity.status(HttpStatus.FORBIDDEN).body(ApiResponse.error("401", "수정 권한이 없습니다."));
		} catch (EntityNotFoundException e) {
			return ResponseEntity.status(HttpStatus.NOT_FOUND).body(ApiResponse.error("404", "해당 멤버를 찾을 수 없습니다."));
	...
	public ResponseEntity<ApiResponse<String>> findMember(@PathVariable Long memberId){
		MemberResponseDto memberResponseDto = memberService.findMember(memberId);
		return ResponseEntity.ok(ApiResponse.success(memberResponseDto.getNickname()));
	}

	@Operation (summary = "멤버 탈퇴하기")
	@DeleteMapping("/leave/{memberId}")
	public ResponseEntity<ApiResponse<String>> leaveMember(@PathVariable Long boardId,
														@PathVariable Long memberId) {
		try {
			memberService.leaveMember(memberId, boardId);
			return ResponseEntity.ok(ApiResponse.success("성공적으로 탈퇴했습니다."));
		} catch (Exception e) {
			return ResponseEntity.badRequest().body(ApiResponse.error("400", e.getMessage()));
		}
	}
